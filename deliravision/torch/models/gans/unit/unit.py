from deliravision.models.gans.unit.models import Generator, Encoder, \
...
class UNIT(AbstractPyTorchNetwork):
    """
    ...
    def closure(model, data_dict: dict, optimizers: dict, losses=None,
        ...
        loss_vals["gen_adversarial_A"] = adv_a.item()
        loss_vals["gen_adversarial_B"] = adv_b.item()

        # calculate kl loss
        kl_a = (preds["mu_a"] ** 2).mean()
        kl_b = (preds["mu_b"] ** 2).mean()
        loss_vals["gen_KL_A"] = kl_a.item()
        loss_vals["gen_KL_B"] = kl_b.item()

        # calculate identity loss
        id_a = losses["pixelwise"](preds["rec_a"], data_dict["imgs_a"])
        id_b = losses["pixelwise"](preds["rec_b"], data_dict["imgs_b"])
        loss_vals["gen_identity_A"] = id_a.item()
        loss_vals["gen_identity_B"] = id_b.item()

        # calculate kl loss for fakes
        kl_fake_a = (preds["mu_fake_a"] ** 2).mean()
        kl_fake_b = (preds["mu_fake_b"] ** 2).mean()
        loss_vals["gen_KL_fake_A"] = kl_fake_a.item()
        loss_vals["gen_KL_fake_B"] = kl_fake_b.item()

        # calculate cycle consistency loss
        cyc_a = losses["pixelwise"](preds["cyc_a"], data_dict["imgs_a"])
        cyc_b = losses["pixelwise"](preds["cyc_b"], data_dict["imgs_b"])
        loss_vals["gen_cycle_consistency_A"] = cyc_a.item()
        loss_vals["gen_cycle_consistency_B"] = cyc_b.item()
        ...
        loss_vals["discr_a_real"] = discr_a_real.item()
        loss_vals["discr_a_fake"] = discr_a_fake.item()
        loss_vals["discr_a_total"] = discr_a.item()
        loss_vals["discr_b_real"] = discr_b_real.item()
        loss_vals["discr_b_fake"] = discr_b_fake.item()
        loss_vals["discr_b_total"] = discr_b.item()
    ...
    def prepare_batch(batch: dict, input_device, output_device):
        return {
            ...
            "data_b": torch.from_numpy(batch["data_b"]).to(
                torch.float).to(input_device)
        }
