...
class AuxiliaryClassifierGANPyTorch(AbstractPyTorchNetwork):
    ...
    def closure(model, data_dict: dict, optimizers: dict, losses=None,
        ...
        loss_vals["auxiliary_generator"] = aux_loss_gen.item()
        loss_vals["total_generator"] = gen_loss.item()
        ...
        loss_vals["adversarial_discriminator"] = adv_loss_discr.item()
        loss_vals["auxiliary_discriminator"] = aux_loss_discr.item()
        loss_vals["total_discriminator"] = discr_loss.item()
        ...
        return {"accuracy": acc}, loss_vals, {k: v.detach()
