from deliravision.models.gans.pixel_da.models import Generator, \
...
class PixelDomainAdaptation(AbstractPyTorchNetwork):
    """
    ...
    def closure(model, data_dict: dict, optimizers: dict, losses=None,
        ...
        loss_vals.update({
            "gen_task_real": gen_task_real.item(),
            "gen_task_fake": gen_task_fake.item(),
            "gen_task_total": gen_task_loss.item(),
            "gen_adversarial": adversarial.item(),
        ...
        loss_vals.update({
            "discr_real": real_loss.item(),
            "discr_fake": fake_loss.item(),
            "discriminator": loss_discr.item()
        })
    ...
    def prepare_batch(batch: dict, input_device, output_device):
        return {
            "data_a": torch.from_numpy(batch["data_a"]).to(torch.float).to(
                input_device),
            "data_b": torch.from_numpy(batch["data_b"]).to(torch.float).to(
                input_device),
            "label_a": torch.from_numpy(batch["label_a"]).to(torch.long).to(
                input_device),
            "label_b": torch.from_numpy(batch["label_b"]).to(torch.long).to(
                input_device),
        }
